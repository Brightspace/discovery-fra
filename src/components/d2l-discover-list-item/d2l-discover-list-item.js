 ...
 * @polymer
 */
class D2lDiscoverListItem extends mixinBehaviors([
	IronResizableBehavior,
	D2L.PolymerBehaviors.FocusableBehavior], DiscoverListItemResponsiveConstants(DiscoverListItemLocalize(MutableData(PolymerElement)))) {
	static get template() {
		return html`
			...
			<style include="d2l-offscreen-shared-styles">
				...
				.d2l-discover-list-item-link-container {
					overflow: hidden;
					display: inline-flex;
					flex-direction: row;
					flex-grow: 1;
				...
				.d2l-discover-list-item-link-text {
				...
				/* P1-shady, P2-shady */
				:host(:dir(rtl)) .d2l-discover-list-item-link-text {
					@apply --d2l-offscreen-rtl
				}
				...
				<div class="d2l-discover-list-item-link-container">
					...
					<div class="d2l-discover-list-item-content">
						...
						<div aria-hidden="true">
							...
							<div hidden$="[[_textPlaceholder]]">
								<h2 class="d2l-discover-list-item-title">
									<d2l-organization-name href="[[_organizationUrl]]" token="[[token]]"></d2l-organization-name>
								</h2>
							</div>
						</div>
	...
	_reset() {
		Object.keys(D2lDiscoverListItem.properties).forEach((key) => {
			this.set(key, D2lDiscoverListItem.properties[key].default);
		});
	}
	...
	_setResponsiveSizes(currentWidth) {
		...
		window.fastdom.mutate(() => {
			...
			const title = this.shadowRoot.querySelector('.d2l-discover-list-item-title');
			...
			const listItemContainer = this.shadowRoot.querySelector('.d2l-discover-list-item-container');
	...
	_onLinkTrigger(event) {
		...
		this.dispatchEvent(new CustomEvent('d2l-discover-trigger', {
			detail: {
	...
	_clampDescription(description) {
		...
		const p = this.shadowRoot.querySelector('.d2l-discover-list-item-description p');
	...
	_setTextPlaceholderStyles(currentConfig) {
		...
		const categoryPlaceholder = this.shadowRoot.querySelector('.d2l-discover-list-item-category-placeholder');
		const titlePlaceholder = this.shadowRoot.querySelector('.d2l-discover-list-item-title-placeholder');
		...
		if (this._footerPlaceholderItems.length !== newFooterCount) {
			...
			beforeNextRender(this, () => {
				const footerPlaceholders = this.shadowRoot.querySelectorAll('.d2l-discover-list-item-footer-placeholder');
...
window.customElements.define('d2l-discover-list-item', D2lDiscoverListItem);
