...
describe('d2l-discover-list-item', () => {
	...
	it('loads element', () => {
		component = fixture('d2l-discover-list-item-empty-fixture');
	...
	[
		{
			...
			beforeEachFn: () => {
				component = fixture('d2l-discover-list-item-href-fixture');
			}
		},
		{
		...
		describe(testCase.name, () => {
			...
			beforeEach(done => {
				...
				window.document.addEventListener('d2l-discover-text-loaded', textLoadedSuccessfulSpy);
				testCase.beforeEachFn();
				afterNextRender(component, done);
			});

			afterEach(() => {
				window.document.removeEventListener('d2l-discover-text-loaded', textLoadedSuccessfulSpy);
			});
		...
		let handler;
		afterEach(() => {
			window.document.removeEventListener('d2l-discover-text-loaded', handler);
			window.document.removeEventListener('d2l-discover-image-loaded', handler);
		});
		...
		it(testCase.name + 'should send text loaded event', done => {
			...
			window.document.addEventListener('d2l-discover-text-loaded', handler);
			testCase.beforeEachFn();
		});
		...
		it(testCase.name + 'should send image loaded event', done => {
			...
			window.document.addEventListener('d2l-discover-image-loaded', handler);
			testCase.beforeEachFn();
		});
	});
	...
	describe('Accessibility', () => {
		...
		it('Organization', done => {
			...
			afterNextRender(component, () => {
				...
				var accessibilityText = component.$$('.d2l-discover-list-item-link-text').innerHTML;
	...
	describe('Responsive Behaviour', () => {
		it('Description is hidden at width 384', done => {
			component = fixture('d2l-discover-list-item-responsive-384-fixture');
			afterNextRender(component, () => {
				expect(component._showDescription).to.be.false;
				var description = component.$$('#d2l-discover-list-item-description');
		...
		it('Description is not hidden at width 385', done => {
			component = fixture('d2l-discover-list-item-responsive-385-fixture');
